        ...
        <div class="card-body d-flex align-items-center gap-3">
            ...
            @if($servidor->fotografia)
                <img src="{{ asset('storage/' . $servidor->fotografia) }}"
                     class="rounded-circle" style="width:60px;height:60px;object-fit:cover;border:2px solid #1565c0;">
            @else
            ...
            <div class="flex-grow-1">
                <div class="fw-bold">{{ $servidor->nombre }} {{ $servidor->apellido_paterno }} {{ $servidor->apellido_materno }}</div>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('servidores.edit', $servidor->id) }}" class="btn btn-sm btn-outline-warning">Editar</a>
                <form action="{{ route('servidores.destroy', $servidor->id) }}" method="POST" onsubmit="return confirm('¿Eliminar?')">
                    @csrf @method('DELETE')
